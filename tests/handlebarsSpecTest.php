...
class HandlebarsSpecTest extends TestCase
{
    ...
    public function testSpecs($spec)
    {
        ...
        foreach ($test_flags as $f) {
            ...
            $helpers = [];
            $helpersList = '';
            foreach (array_merge((isset($spec['globalHelpers']) && is_array($spec['globalHelpers'])) ? $spec['globalHelpers'] : [], (isset($spec['helpers']) && is_array($spec['helpers'])) ? $spec['helpers'] : []) as $name => $func) {
                if (!isset($func['php'])) {
            ...
            try {
                $partials = isset($spec['globalPartials']) ? $spec['globalPartials'] : [];
            ...
            try {
                $ropt = ['debug' => Runtime::DEBUG_ERROR_EXCEPTION];
                if (isset($spec['options']['data'])) {
